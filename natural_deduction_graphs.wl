...
NodeLabel[rule_, expr_] := Placed[
  Column[{
    Style[rule, 12, FontColor -> GrayLevel[0.3]],
    Style[FormatNode[expr], 16, FontFamily -> "Times"]
  }, Frame -> All, Alignment -> Center, FrameStyle -> GrayLevel[0.7], 
  Background -> {{GrayLevel[0.95], GrayLevel[0.9]}}
  ],
Center];

GetSecondFromNodeLabel[Placed[Column[{_, Style[t_, ___]}, ___], _]] := t

ProofType[g_Graph] := 
 RootNode[g] /. PropertyValue[g, VertexLabels] // GetSecondFromNodeLabel

$ProofGraphStyle = {
  EdgeStyle -> GrayLevel[0,0.3],
  EdgeShapeFunction -> ({Arrowheads[{{Medium, 0.55}}], Arrow[#]}&)
};
 ...
 With[{plhs = ProofToGraph[lhs, ctx], prhs = ProofToGraph[rhs, ctx]}, 
  With[{rlhs = RootNode[plhs], rrhs = RootNode[prhs], nvx = NewVertexName[]}, 
   EdgeAdd[VertexAddWithLabel[GraphUnionWithLabels[plhs, prhs], 
     Labeled[nvx, 
      NodeLabel["\[And]I", Inactive[And][ProofType[plhs], ProofType[prhs]]]]], {rlhs \[DirectedEdge] nvx, rrhs \[DirectedEdge] nvx}]]]
  ...
  With[{r = RootNode[prf], nvx = NewVertexName[]},
   EdgeAdd[
    VertexAddWithLabel[prf, 
     Labeled[nvx, NodeLabel["\[And]EL", ProofType[prf]//FirstPart]]],
     r \[DirectedEdge] nvx]]]
...
ProofToGraph[AndElimRight[pand_], ctx_] :=
 With[{prf = ProofToGraph[pand, ctx]}, 
  With[{r = RootNode[prf], nvx = NewVertexName[]},
   EdgeAdd[
    VertexAddWithLabel[prf, 
     Labeled[nvx, NodeLabel["\[And]ER", ProofType[prf]//SecondPart]]],
     r \[DirectedEdge] nvx]]]
 ...
 With[{prf = ProofToGraph[plhs, ctx]}, 
  With[{r = RootNode[prf], nvx = NewVertexName[]},
   EdgeAdd[
    VertexAddWithLabel[prf, 
     Labeled[nvx, 
      NodeLabel["\[Or]IL", Inactive[Or][ProofType[prf], trhs]]]], 
    r \[DirectedEdge] nvx]]]
 ...
 With[{prf = ProofToGraph[prhs, ctx]}, 
  With[{r = RootNode[prf], nvx = NewVertexName[]},
   EdgeAdd[
    VertexAddWithLabel[prf, 
     Labeled[nvx, 
      NodeLabel["\[Or]IR", Inactive[Or][tlhs, ProofType[prf]]]]], 
    r \[DirectedEdge] nvx]]]
  ...
  With[{rimp = RootNode[prfi], rhyp = RootNode[prfh], nvx = NewVertexName[]},
   EdgeAdd[
    VertexAddWithLabel[GraphUnionWithLabels[prfi, prfh], 
     Labeled[nvx, 
      NodeLabel["\[Implies]E", ImpliesConclusion[ProofType[prfi]]]]], {rimp \[DirectedEdge] nvx, rhyp \[DirectedEdge] nvx}]]]
 ...
 With[{prf = ProofToGraph[pf, ExtendContext[ctx, nm -> LocalHyp[nm, hyp]]]}, 
  With[{r = RootNode[prf], nvx = NewVertexName[]},
   EdgeAdd[
    VertexAddWithLabel[prf, 
     Labeled[nvx, 
      NodeLabel["\[Implies]I  [" <> ToString[nm] <> "]", 
        Inactive[Implies][hyp, ProofType[prf]]]]], 
    r \[DirectedEdge] nvx]]]

ProofToGraph[LocalHyp[nm_, tp_], ctx_] :=
 Graph[{Labeled[NewVertexName[], 
    NodeLabel["Hyp: " <> ToString[nm], tp]]}, {}]
 ...
 With[{prfo = ProofToGraph[por, ctx], prfl = ProofToGraph[plhs, ctx], 
  ...
  With[{ror = RootNode[prfo], rlhs = RootNode[prfl], rrhs = RootNode[prfr], 
   ...
   EdgeAdd[VertexAddWithLabel[
     ...
     Labeled[nvx, 
      NodeLabel["\[Or]E", ImpliesConclusion[ProofType[prfl]]]]], {ror \[DirectedEdge] nvx, rlhs \[DirectedEdge] nvx, 
     rrhs \[DirectedEdge] nvx}]]]

ProofToGraph[TrueIntro, ctx_] := 
 Graph[{Labeled[NewVertexName[], NodeLabel["TI", True]]}, {}]
